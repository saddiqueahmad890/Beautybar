            ...
            <div class="col-sm-6">
                <ol class="breadcrumb float-sm-right">
                    <li class="breadcrumb-item">
                        <a href="{{ route('doctor-details.index') }}">@lang('Employee ')</a></li>
                    <li class="breadcrumb-item active">@lang('Employee  Info')</li>
                </ol>
            </div>
        </div>
    </div>
    ...
    <div class="col-md-12">
        <div class="card">
            <div class="card-header bg-info">
                <h3 class="card-title">@lang('Employee  Info')</h3>
...
{{-- <div class="card">
    ...
    <div class="card-body">
        ...
        <ul class="nav nav-tabs" id="myTab" role="tablist">
            ...
            {{-- <li class="nav-item">
                <a class="nav-link" id="profile-tab" data-toggle="tab" href="#tab-exam-investigations" role="tab"
                    aria-controls="profile" aria-selected="false">Exam & Investigations</a>
            </li> 
            <li class="nav-item">
                ...
                <table class="table table-striped" id="laravel_datatable">
                    ...
                    <tbody>
                        @foreach ($patientAppointments as $patientAppointment)
                            <tr>
                                <td>{{ $patientAppointment->appointment_number  }}</td>
                                <td>{{ isset($patientAppointment->user->name) ? $patientAppointment->user->name : "-"  }}</td>
                ...
                <table class="table table-striped" id="laravel_datatable">
                    ...
                    <tbody>
                        @foreach ($examInvestigations as $examInvestigation)
                            <tr>
                                <td>{{ $examInvestigation->examination_number  }}</td>
                                <td>{{ isset ($examInvestigation->PatientAppointment->appointment_number) ? $examInvestigation->PatientAppointment->appointment_number : "-"  }}</td>
                                <td>{{ isset($examInvestigation->patient->name) ? $examInvestigation->patient->name : '-' }}</td>
                    ...
                    <tbody>
                        @foreach ($patientTreatmentPlans as $patientTreatmentPlan)
                            <tr>
                                ...
                                <td>{{ isset ($patientTreatmentPlan->examinvestigation->examination_number) ? $patientTreatmentPlan->examinvestigation->examination_number : "-"  }}</td>
                                <td>{{ isset($patientTreatmentPlan->patient->name) ? $patientTreatmentPlan->patient->name : '-' }}</td>
                    ...
                    <tbody>
                        @foreach ($prescriptions as $prescription)
                            <tr>
                                ...
                                <td>{{ isset ($prescription->examinvestigations->examination_number) ? $prescription->examinvestigations->examination_number : "-"  }}</td>
