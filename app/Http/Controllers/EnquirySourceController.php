...
class EnquirySourceController extends Controller
{
    ...
    {
        $enquirysource = $this->filter($request)->orderBy('id', 'desc')->paginate(10);
    ...
    /**
     * Apply filters to the EnquirySource query based on request parameters.
     *
    ...
    {
        $query = EnquirySource::query();

        if ($request->filled('source_name')) {
            $query->where('source_name', 'like', '%' . $request->input('source_name') . '%');
        }
